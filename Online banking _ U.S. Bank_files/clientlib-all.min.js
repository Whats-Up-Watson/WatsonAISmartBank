setTimeout(function(){var a=document.querySelectorAll("span.whitearrowtextButton, span.whitearrowsmallTextButton, span.whitetextButton, span.whitesmallTextButton, span.arrowtextButton, span.arrowsmallTextButton, span.textButton, span.smallTextButton, span.redSolidButtonSmall, span.redSolidButtonDefault, span.redSolidButtonLarge, span.blueSolidButtonSmall, span.blueSolidButtonDefault, span.blueSolidButtonLarge, span.redLinedButtonSmall, span.redLinedButtonDefault, span.redLinedButtonLarge, span.blueLinedButtonSmall, span.blueLinedButtonDefault, span.blueLinedButtonLarge");
a.forEach(function(f){var b=f;
var e=b.getAttribute("class");
var c=b.parentElement;
var g=c.getAttribute("href")||"";
var h=c.getAttribute("aria-label");
if(!h){h=c.getAttribute("title")
}if(h){h=" aria-label = '"+h+"' "
}var d=document.createElement("BUTTON");
d.innerHTML="<span class='pAlternative'>"+f.innerText+"</span>";
d.className=e+" "+h;
d.setAttribute("role","button");
d.onclick=function(){location.href=g
};
c.replaceWith(d)
})
});
document.addEventListener("DOMContentLoaded",function(){$("button.whitearrowtextButton, button.whitearrowsmallTextButton, button.whitetextButton, button.whitesmallTextButton, button.arrowtextButton, button.arrowsmallTextButton, button.textButton, button.smallTextButton, button.redSolidButtonSmall, button.redSolidButtonDefault, button.redSolidButtonLarge, button.blueSolidButtonSmall, button.blueSolidButtonDefault, button.blueSolidButtonLarge, button.redLinedButtonSmall, button.redLinedButtonDefault, button.redLinedButtonLarge, button.blueLinedButtonSmall, button.blueLinedButtonDefault, button.blueLinedButtonLarge").on("keydown",function(a){if(a.keyCode==40||a.keyCode==32){$(this).next("button, a, input, select").focus();
a.preventDefault()
}else{if(a.keyCode==38){$(this).prev("button, a, input, select").focus();
a.preventDefault()
}}})
},false);
$(document).ready(function(){const a=document.querySelectorAll("[data-component='tile']"),d=a.length;
for(i=0;
i<d;
i++){c(i)
}$("button.usb-tile--toggle").append("<span></span>");
function c(g){a[g].setAttribute("data-state","not-toggled");
a[g].addEventListener("click",function(h){b(h,a[g],"toggled","not-toggled")
});
a[g].addEventListener("mouseenter",function(h){e(a[g])
});
a[g].addEventListener("mouseleave",function(h){f(a[g])
})
}function b(k,j,n,m){console.log(k.target.dataset.component);
if(k.target.dataset.component){j.setAttribute("data-state",j.getAttribute("data-state")===n?m:n);
j.classList.toggle("toggled");
var g=j.querySelectorAll("a[aria-hidden]");
$(g).each(function(){$(this).attr("tabindex",j.getAttribute("data-state")===n?"0":"-1");
$(this).attr("aria-hidden",j.getAttribute("data-state")===n?"true":"false")
});
var l=j.querySelectorAll("[aria-hidden]");
$(l).each(function(){$(this).attr("aria-hidden",j.getAttribute("data-state")===n?"false":"true");
if(j.getAttribute("data-state")==="not-toggled"){console.log("Add Here");
if($(j).hasClass("parentTileSection")){}else{$(j).find(".usb-tile--title").css({transition:".9s all ease-in-out","padding-bottom":"10px"})
}$(j).find("button.usb-tile--toggle").attr("aria-label","Expand Content");
$(j).find("button.usb-tile--toggle").attr("aria-expanded","false");
$(j).find(".usb-tile--description").attr("aria-hidden","true").attr("tabindex","");
if($(window).width()<=1024){$(".usb-tile--description").attr("aria-hidden","true")
}}else{console.log("Remove Here");
$(j).find(".usb-tile--title").css("padding-bottom",($(j).find(".usb-tile--description p").height())+15);
$(j).find("button.usb-tile--toggle").attr("aria-label","Collapse Content");
$(j).find("button.usb-tile--toggle").attr("aria-expanded","true");
$(j).find(".usb-tile--description").attr("aria-hidden","false").attr("tabindex","-1")
}});
var h=j.querySelectorAll("[tabindex]");
$(h).each(function(){})
}}function e(h){if($(window).width()>991){$(h).addClass("parentTileSection")
}if(h.getAttribute("data-state")=="not-toggled"){$(h).find(".usb-tile--title").css("padding-bottom",($(h).find(".usb-tile--description p").height())+45);
console.log("parentTileSection Addedd ==>");
$(h).find(".wm-heading-xs ").css({"Font-family":"Helvetica Neue LT Std","font-size":"28px","line-height":"35px","font-weight":"bold",color:"White"});
$(h).find(".usb-tile--title").css({transition:".4s all ease-in-out"});
var j=h.querySelectorAll("[aria-hidden]");
$(j).each(function(){$(this).attr("aria-hidden","false")
});
var g=h.querySelectorAll("[tabindex]");
$(g).each(function(){$(this).attr("tabindex","0")
});
$(".usb-tile--description").attr("aria-hidden","").attr("tabindex","");
if($(window).width()<=1024){$(".usb-tile--description ").css({transition:"1s all ease-in-out"});
$(".usb-tile--description").attr("aria-hidden","true")
}}}function f(h){$(h).removeClass("parentTileSection");
var j=h.querySelectorAll("[aria-hidden]");
$(j).each(function(){$(this).attr("aria-hidden","true")
});
var g=h.querySelectorAll("[tabindex]");
$(j).each(function(){$(this).attr("tabindex","-1")
});
$(h).find(".wm-heading-xs ").css({"Font-family":"Helvetica Neue LT Std","font-size":"28px","line-height":"35px","font-weight":"bold",color:"white"});
if(h.getAttribute("data-state")=="toggled"){var l=h.querySelectorAll("[aria-hidden]");
$(l).each(function(){$(this).attr("aria-hidden","false")
});
var k=h.querySelectorAll("[tabindex]");
$(l).each(function(){$(this).attr("tabindex","0")
})
}else{$(h).find(".usb-tile--title").css({transition:".9s all ease-in-out","padding-bottom":"10px"});
$(h).find(".usb-tile--description ").css({transition:".6s all ease-in-out"})
}$(".usb-tile--description").attr("tabindex","").attr("aria-hidden","true")
}$(".usb-tile--description sup a").attr("tabindex","-1").attr("aria-hidden","true");
if((navigator.userAgent.match(/iPhone/i))||(navigator.userAgent.match(/iPad/i))){$(".usb-tile-shell .usb-tile--title").bind("touchstart",function(g){g.preventDefault();
g.stopPropagation()
})
}});
function lf_onMessage(c){if(typeof c.data==="string"&&c.data.charAt(0)==="{"){var b=JSON.parse(c.data.replace(new RegExp('}"',"g"),"}").replace(new RegExp('"{',"g"),"{").trim())
}if(!c||!b||(b.action!=="resetTimer"&&b.action!=="scrollTop"&&b.action!=="resize"&&typeof b.action==="undefined")){return
}if(b.action==="resize"){var a=document.getElementsByClassName("lf_tool_frame_"+b.iframeID);
if(a.length===0){a=document.getElementById("lf_tool_frame")
}else{a=a[0]
}if(a!==null){a.height=b.height
}}else{if(b.action==="scrollTop"){var d=setInterval(function(){clearInterval(d);
...
}}$(function(){var a=$('[class*="showHideId"]'),b=0;
if(a){a.each(function(f,j){j=$(j);
var g=j.find(".show-hide-container"),c=j.find(".USBPanel"),h=j.find(".show-hide-button"),e=0,d="";
if((/usbank\.com/i.test(window.location.href)||/wcmmode=disabled/i.test(window.location.search))&&c.hasClass("is-collapsed")){setTimeout(function(){g.attr({"aria-hidden":"true"}).prop({hidden:true})
},500)
}else{e=e+1;
g.addClass("is-visible").removeClass("is-hidden").attr({"aria-hidden":"false"}).prop({hidden:false});
c.addClass("is-expanded").removeClass("is-collapsed");
h.attr({"aria-expanded":"true"});
d="showOnLoad"
}e=0;
if(d==="showOnLoad"){e=1
}j.removeAttr("data-text");
h.on("click",function(k){console.log("Clicked \n",$(k.target));
if((e%2)===0){g.addClass("is-visible").removeClass("is-hidden").removeClass("showHideHide").attr({"aria-hidden":"false"}).prop({hidden:false});
c.addClass("is-expanded").removeClass("is-collapsed");
h.attr({"aria-expanded":"true"});
setTimeout(function(){$(k.target).focus()
},200)
}else{g.addClass("is-hidden").removeClass("is-visible").attr({"aria-hidden":"true"}).prop({hidden:true});
c.addClass("is-collapsed").removeClass("is-expanded");
...
$(".USBPanel-head").each(function(){var k=$(this).find(".USBPanel-title").find("span").text();
if($(this).find("H1").length){$(this).attr("aria-label",k+" heading level 1").attr("role","button")
}if($(this).find("H2").length){$(this).attr("aria-label",k+" heading level 2").attr("role","button")
}if($(this).find("H3").length){$(this).attr("aria-label",k+" heading level 3").attr("role","button")
}if($(this).find("H4").length){$(this).attr("aria-label",k+" heading level 4").attr("role","button")
}if($(this).find("H5").length){$(this).attr("aria-label",k+" heading level 5").attr("role","button")
}if($(this).find("H6").length){$(this).attr("aria-label",k+" heading level 6").attr("role","button")
}})
})
}});
$(document).ready(function(){$(".USBPanel-title").each(function(a){var b=$(this).parent().find(".arrow-icon");
b.css({top:(($(this).height())/2-8)+"px"})
})
});
...
}),this.on("submit.validate",function(c){function h(){var j,b;
return !g.settings.submitHandler||(g.submitButton&&(j=e("<input type='hidden'/>").attr("name",g.submitButton.name).val(e(g.submitButton).val()).appendTo(g.currentForm)),b=g.settings.submitHandler.call(g,g.currentForm,c),g.submitButton&&j.remove(),void 0!==b&&b)
...
var m=e.data(this.form,"validator"),l="on"+j.type.replace(/^validate/,""),k=m.settings;
k[l]&&!e(this).is(k.ignore)&&k[l].call(m,this,j)
...
},element:function(a){var o,n,m=this.clean(a),l=this.validationTargetFor(m),k=this,j=!0;
return void 0===l?delete this.invalid[m.name]:(this.prepareElement(l),this.currentElements=e(l),n=this.groups[l.name],n&&e.each(this.groups,function(g,c){c===n&&g!==l.name&&(m=k.validationTargetFor(k.clean(k.findByName(g))),m&&m.name in k.invalid&&(k.currentElements.push(m),j=k.check(m)&&j))
}),o=this.check(l)!==!1,j=j&&o,o?this.invalid[l.name]=!1:this.invalid[l.name]=!0,this.numberOfInvalids()||(this.toHide=this.toHide.add(this.containers)),this.showErrors(),e(a).attr("aria-invalid",!o)),j
...
}}},defaultMessage:function(a,j){"string"==typeof j&&(j={method:j});
var h=this.findDefined(this.customMessage(a.name,j.method),this.customDataMessage(a,j.method),!this.settings.ignoreTitle&&a.title||void 0,e.validator.messages[j.method],"<strong>Warning: No message defined for "+a.name+"</strong>"),g=/\$?\{(\d+)\}/g;
return"function"==typeof h?h=h.call(this,j.parameters,a):g.test(h)&&(h=e.validator.format(h.replace(g,"{$1}"),j.parameters)),h
},formatAndAdd:function(h,g){var j=this.defaultMessage(h,g);
this.errorList.push({message:j,element:h,method:g.method}),this.errorMap[h.name]=j,this.submitted[h.name]=j
...
},errorsFor:function(a){var j=this.escapeCssMeta(this.idOrName(a)),h=e(a).attr("aria-describedby"),g="label[for='"+j+"'], label[for='"+j+"'] *";
...
},normalizeAttributeRule:function(h,g,k,j){/min|max|step/.test(k)&&(null===g||/number|range|text/.test(g))&&(j=Number(j),isNaN(j)&&(j=void 0)),j||0===j?h[k]=j:g===k&&"range"!==g&&(h[k]=!0)
},attributeRules:function(a){var m,l,k={},j=e(a),h=a.getAttribute("type");
for(m in e.validator.methods){"required"===m?(l=a.getAttribute(m),""===l&&(l=!0),l=!!l):l=j.attr(m),this.normalizeAttributeRule(k,h,m,l)
}return k.maxlength&&/-1|2147483647|524288/.test(k.maxlength)&&delete k.maxlength,k
},dataRules:function(a){var m,l,k={},j=e(a),h=a.getAttribute("type");
for(m in e.validator.methods){l=j.data("rule"+m.charAt(0).toUpperCase()+m.substring(1).toLowerCase()),this.normalizeAttributeRule(k,h,m,l)
}return k
...
return g&&g.length>0
}return this.checkable(j)?this.getLength(a,j)>0:a.length>0
...
},max:function(h,g,j){return this.optional(g)||h<=j
...
return this.settings.onfocusout&&g.not(".validate-equalTo-blur").length&&g.addClass("validate-equalTo-blur").on("blur.validate-equalTo",function(){e(j).valid()
}),a===g.val()
},remote:function(a,p,o,n){if(this.optional(p)){return"dependency-mismatch"
}n="string"==typeof n&&n||"remote";
var m,l,k,j=this.previousValue(p,n);
return this.settings.messages[p.name]||(this.settings.messages[p.name]={}),j.originalMessage=j.originalMessage||this.settings.messages[p.name][n],this.settings.messages[p.name][n]=j.message,o="string"==typeof o&&{url:o}||o,k=e.param(e.extend({data:a},o.data)),j.old===k?j.valid:(j.old=k,m=this,this.startRequest(p),l={},l[p.name]=a,e.ajax(e.extend(!0,{mode:"abort",port:"validate"+p.name,dataType:"json",data:l,context:m.currentForm,success:function(b){var s,r,q,c=b===!0||"true"===b;
m.settings.messages[p.name][n]=j.originalMessage,c?(q=m.formSubmitted,m.resetInternals(),m.toHide=m.errorsFor(p),m.formSubmitted=q,m.successList.push(p),m.invalid[p.name]=!1,m.showErrors()):(s={},r=b||m.defaultMessage(p,{method:n,parameters:a}),s[p.name]=j.message=r,m.invalid[p.name]=!0,m.showErrors(s)),j.valid=c,m.stopRequest(p,c)
}},o)),"pending")
}}});
var d,f={};
return e.ajaxPrefilter?e.ajaxPrefilter(function(g,c,j){var h=g.port;
...
$(document).ready(function(){jQuery.validator.addMethod("lettersonly",function(b,a){return this.optional(a)||/^[a-z]+$/i.test(b)
},"Letters only please");
$("#mortgage_form_second").validate({submitHandler:function(){var a=$("#mortgage_form_second");
a.submit();
d()
},rules:{phone:{required:true,minlength:14},"00N60000002qTSn":{minlength:14},email:{required:true,email:true,emailchecking:true},street:{required:false,minlength:5},first_name:{required:true,minlength:3},last_name:{required:true,minlength:2}},messages:{phone:{required:"Error: Phone number is a required field.",minlength:"Error: Phone number should be 10 numeric digits and should not start with 0 or 1."},"00N60000002qTSn":{minlength:"Phone number should be 10 numeric digits and should not start with 0 or 1."},first_name:{required:"Error: First name is a required field."},last_name:{required:"Error: Last name is a required field."},email:{required:"Error: Email address is a required field."}}});
var g=document.getElementById("email");
if(g){g.addEventListener("keydown",function(a){if(g.value.length===0&&a.which===189){a.preventDefault()
}})
}$("#email").focusout(function(){console.log("email focus out");
var a=$("#email").val();
while(a.charAt(0)==="-"){a=a.substr(1)
}document.getElementById("email").value=a
});
jQuery.validator.addMethod("emailchecking",function(b,a){return this.optional(a)||/^[a-zA-Z][\w\.-]*[a-zA-Z0-9]@[a-zA-Z0-9][\w\.-]*[a-zA-Z0-9]\.[a-zA-Z][a-zA-Z\.]*[a-zA-Z]$/.test(b);
console.log(b);
console.log(a)
},"Error: Please enter valid email address.");
function d(){setTimeout(function(){if(($("#first_name").hasClass("error"))){$(".infoFirst").show()
}if(($("#first_name").hasClass("valid"))){$(".infoFirst").hide()
}if(($("#last_name").hasClass("error"))){$(".infoLast").show()
}if(($("#last_name").hasClass("valid"))){$(".infoLast").hide()
}if(($("#email").hasClass("error"))){$(".infoEmail").show()
}if(($("#email").hasClass("valid"))){$(".infoEmail").hide()
}if(($("#phone").hasClass("error"))){$(".infoPhone").show()
}if(($("#phone").hasClass("valid"))){$(".infoPhone").hide()
}console.log($("#phone-error").hasClass("error"));
if(($("#phone").hasClass("error"))||($("#last_name").hasClass("error"))||($("#first_name").hasClass("error"))){if(typeof(onSubmitError)==="function"){onSubmitError(false)
}$(".error-content-container").addClass("bg-error");
$(".error-content-container").attr("tabindex",0);
$(".error-content-container").focus()
}else{$(".error-content-container").hide()
}},10)
}$("#mortgage_form_second #phone").focusout(function(){console.log("focus out expenses");
var k=$("#mortgage_form_second #phone");
var h=k.val();
var b=(h).replace("(","").replace("$","").replace("&","").replace(")","").replace(" ","").replace("-","").replace("-","").replace("x","").replace("x","").split().join();
var j=b;
j=j.slice(1,10);
var a=b.replace(/(\d\d\d)(\d\d\d)(\d\d\d\d)(\d\d\d\d)/,"($1)-$2-$3x$4");
if(a!=0&&b!=0&&j>0){if(b.length<=10){b=b.replace(/(\d\d\d)(\d\d\d)(\d\d\d\d)/,"($1)-$2-$3");
k.val(b)
}if(b.length>10){b=b.replace(/(\d\d\d)(\d\d\d)(\d\d\d\d)(\d\d\d\d)/,"($1)-$2-$3x$4");
k.val(b)
}}else{if(a==0&&a==0&&b==0&&j==0){k.val("")
}else{k.val("")
}}});
$("#mortgage_form_second #00N60000002qTSn").focusout(function(){console.log("focus out expenses");
var h=$("#mortgage_form_second #00N60000002qTSn");
var k=h.val();
var b=(k).replace("(","").replace("$","").replace("&","").replace(")","").replace(" ","").replace("-","").replace("-","").replace("x","").replace("x","").split().join();
var j=b;
j=j.slice(1,10);
var a=b.replace(/(\d\d\d)(\d\d\d)(\d\d\d\d)(\d\d\d\d)/,"($1)-$2-$3x$4");
if(a!=0&&b!=0&&j>0){if(b.length<=10){b=b.replace(/(\d\d\d)(\d\d\d)(\d\d\d\d)/,"($1)-$2-$3");
...
h.val(b)
}}else{if(a==0&&a==0&&b==0&&j==0){h.val("")
}else{h.val("")
}}});
$("#mortgage_form_second #first_name, #mortgage_form_second #last_name").bind("keypress",function(h){if(h.charCode!=0){var b=new RegExp("^[a-zA-Z ]+$");
var a=String.fromCharCode(!h.charCode?h.which:h.charCode);
if(!b.test(a)){h.preventDefault();
return false
}}});
$("#mortgage_form_second #phone").bind("keypress",function(h){if(h.charCode!=0){var b=new RegExp("^[0-9]+$");
var a=String.fromCharCode(!h.charCode?h.which:h.charCode);
if(!b.test(a)){h.preventDefault();
return false
}}});
$("#mortgage_form_second #00N60000002qTSn").bind("keypress",function(h){if(h.charCode!=0){var b=new RegExp("^[0-9]+$");
var a=String.fromCharCode(!h.charCode?h.which:h.charCode);
if(!b.test(a)){h.preventDefault();
return false
}}});
$("#aboutYou_cancelApp").click(function(){window.history.back();
return false
});
function f(){var a=document.getElementById("00N60000002r12c");
if(typeof(a)!="undefined"&&a!=null){function h(j){var k=(Math.random().toString(16)+"000000000").substr(2,8);
return j?"-"+k.substr(0,4)+"-"+k.substr(4,4):k
}var b=h()+h(true)+h(true)+h();
document.getElementById("00N60000002r12c").value=b;
return true
}}if($("form#mortgage_form_second").length>0){f();
$("#email_no").attr("checked","checked");
var e=$("input#00N60000002r12c").val();
var c=$("input[name=retURL]").val();
$("input[name=retURL]").val(c+"?"+e)
}});
$(function(){var j=$(".disclosureAccessibility"),f=$(".disclosureReturnNumber"),g=$(".disclosureReturn"),b=$(".disclosureOrderedListLiClass"),d=$("sup a"),c=$("a sup"),a=$(window).width();
if(window.location.href.match(/:4502|uat/)){console.log("%cDisclosures - Version 1.18","color: rebeccapurple; font-weight: bold;")
}function h(k,m,l){if(k){if(typeof m==="number"&&!l){l=m;
m=k
}m=m||k;
l=l||0;
$("html,body").animate({scrollTop:$(m).offset().top-l},200,"swing",function(){k.focus()
})
}}if(f){f.click(function(m){var l=$(m.target).nextAll(".discAnchor").attr("id"),k=$(m.target).attr("href"),n=$("#"+k.substring(1,k.size));
m.preventDefault();
if(!$("body").find("a").hasClass(l)){$("a[data-name="+l+"]").addClass(l)
}if(n){h("."+l,n)
}})
}if(g){g.on("click",function(k){var m=$(k.target)||$(this),l=$("body").find(m.attr("href")+":visible:first");
if(m.hasClass("clickedlinkFocus")){l=$("body").find("a.linkFocus")
}k.preventDefault();
if(l){h(l)
}});
if(b){b.each(function(l,k){var n=$(k).attr("id").split("__");
var m=".disclosureParentDivClass__"+n[1];
if(a>=1200){$(m).prepend("<span id='"+n[1]+"' tabindex='-1' class='sr-only'></span>")
}else{$(m).prepend("<a id='"+n[1]+"' class='sr-only disclosure_link'>Start of disclosure content</a>");
$(m).find("a.disclosure_link").on("click",function(o){o.preventDefault();
o.stopPropagation()
})
}$(m).find("p").attr("id","discl__"+n[1]).attr("tabindex","-1")
});
if(j){var e=navigator.userAgent.toLowerCase().indexOf("android")>-1;
if(e){j.addClass("sr-only-disclosure-android")
}j.addClass("sr-only")
}}}setTimeout(function(){if(d){d.each(function(m,k){var l=$(k).attr("href");
if(typeof l==="undefined"){l="#"
}if(l){m=l.substring(1,l.size);
var n=$(this).parent().parent().find("label").text();
$(this).attr({"aria-label":n+" Footnote "+m,"data-name":m,id:"returnTo__"+m})
}});
d.on("click",function(n){var r=$(n.target),k=r.attr("href"),l=k.substring(1,k.size),o=".disclosureParentDivClass__"+l,q=$(".disclosureReturn"),m=$(".disclosureReturnId__"+l),p=$(o).find("p").first();
n.preventDefault();
d.removeClass("linkFocus").filter(r).addClass("linkFocus");
if(q){q.removeClass("clickedlinkFocus").filter(m).addClass("clickedlinkFocus")
}if(o&&p){h(p,20)
}})
}},1000);
if(c){c.each(function(n,m){var k=$(m).closest("a"),l=k.attr("href");
if(l.startsWith("#")){$(m).wrapInner('<a href="'+l+'"></a>');
$(k).replaceWith(m.innerHTML)
}})
}});
$(document).ready(function(){function c(g,f){if(!f){f=window.location.href
}g=g.replace(/[\[\]]/g,"\\$&");
var j=new RegExp("[?&]"+g+"(=([^&#]*)|&|#|$)"),h=j.exec(f);
if(!h){return null
}if(!h[2]){return""
}return decodeURIComponent(h[2].replace(/\+/g," "))
}var d=c("FirstName");
var a=c("LastName");
var e="mortgage consultant";
if(d){e=d+" "+a
}var b="<h1>Request a call from "+e+"</h1>";
$(".mlo-form-title").html(b)
});